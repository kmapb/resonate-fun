...
@resonate.register
def model_to_pipeline(model_name: str):
    return pipeline("text-generation", model=model_name, model_kwargs={"torch_dtype": torch.bfloat16}, device="mps:0")


@resonate.register
def complete(pipe, sysprompt: str, user_query: str, max_new_tokens: int=512):
    messages = [
        {
            "role": "system",
            "content": sysprompt,
...
if __name__ == "__main__":
    pipe = model_to_pipeline("google/gemma-3-4b-it")
    ...
    while True:
        ...
        print(complete(pipe, sysprompt, query))
